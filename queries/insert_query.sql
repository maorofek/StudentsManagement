INSERT INTO `newdb`.`students`
(`department`,`email`,`first_name`,`gpa`,`last_name`)
VALUES
('Arts','a@gmail.com','a',85,'aaa'),
('Science','b@gmail.com','b',75,'bbb'),
('Engineering','c@gmail.com','c',99,'ccc'),
('Business','d@gmail.com','d',95,'ddd'),
('Science','e@gmail.com','e',68,'eee'),
('Business','f@gmail.com','f',85,'fff'),
('Engineering','g@gmail.com','g',97,'ggg'),
('Law','h@gmail.com','h',73,'hhh'),
('Humanities','i@gmail.com','i',79,'iii'),
('Medicine','j@gmail.com','j',86,'jjj')

